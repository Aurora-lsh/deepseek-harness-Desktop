...
interface KoffiDecode {
  ...
  string16(pointer: unknown, length?: number | bigint): string
}
interface Koffi {
...
/**
 * Read a NUL-terminated UTF-16 string at a native address without creating
 * an external ArrayBuffer, which Electron forbids. The direct string decoder
 * reads the address itself; generic `decode(address, 'str16')` would treat the
 * path bytes as another pointer.
 */
