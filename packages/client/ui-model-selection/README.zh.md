...
间接影响。两个入口都通过仅供普通会话使用的 `session.selectModel` RPC 提交完整的 `ModelSelection`；Host 会在下一次提示词组装边界对其进行快照，因此后续请求采用所选提供方、模型与推理强度，而运行中的步骤保留已组装选择。切换只属于当前会话；只有当请求头记录一次实际采用该选择的请求后，选择才会随会话持久化。菜单交互不会添加提示词内容。
