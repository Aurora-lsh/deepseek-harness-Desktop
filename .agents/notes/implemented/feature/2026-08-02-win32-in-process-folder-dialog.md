...
- The packaged-binary arm — the packaged executable spawning itself as the dialog entry — is not exercised by any automated test: the source plane and the built `lib/worker.cjs` under plain node are covered, while completing a real selection under Electron Node mode remains a Windows release smoke until the Windows CI roadmap supplies GUI automation.
