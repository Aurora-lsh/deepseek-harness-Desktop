...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - `apps/desktop` 通过回环 Web 载体复用同一套 Web Profile；[Windows 桌面壳笔记](2026-08-17-windows-desktop-shell.md)记录打包职责，IPC fetch 载体仍是未来的物理传输选项。
...
| 承载层 | `dsh-host-webserver` | Web HTTP 与 upgrade：静态服务 + `/api/*`→handler 转发 + WebSocket upgrade route + close 语义；插件 bundle 端点 + `__DSH_BOOT__` manifest（元数据清单）注入（由 web 插件注册表供给） | 浏览器访问与通过回环地址连接的 Electron 桌面壳；零 workspace 依赖（注册表经结构注入到达） |
