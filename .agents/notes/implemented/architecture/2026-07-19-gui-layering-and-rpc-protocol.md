...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/desktop` reuses the same Web profile over the loopback Web carrier; the [Windows desktop shell note](2026-08-17-windows-desktop-shell.md) records the packaging boundary, while an IPC fetch carrier remains a future transport option.
...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Browser access and the Electron desktop shell over loopback; zero workspace dependencies (the registry arrives by structural injection) |
