...
| [`@img/sharp-win32-x64`](https://github.com/lovell/sharp) | Apache-2.0 AND LGPL-3.0-or-later |
...
| [`node-addon-require-builtin-win32-x64-msvc`](https://www.npmjs.com/package/node-addon-require-builtin-win32-x64-msvc) | MIT |
...
The Windows desktop installer includes `@img/sharp-win32-x64` 0.35.3 so the attachment service can load Sharp and its dynamically linked libvips libraries on Windows x64. Distribution authorization is limited to this exact package identity, version, and declared `Apache-2.0 AND LGPL-3.0-or-later` terms. The installed package retains its `LICENSE`, `README.md`, version manifest, and shared libraries; changing any reviewed field requires a new distribution decision.
